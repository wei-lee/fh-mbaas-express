  ...
  connect.router(function (app) {
    ...
    app.post('/sync/:datasetId', function(req, res) {
      ...
      $fh.sync.invoke(dataset_id, params, function(err, ok) {
        return endResponseCallback(res, err, ok);
      });
    });
    ...
    app.post('/:func', function (req, res) {
      ...
      async.series([
        ...
        function (callback) {
          ...
          if(err){
            endResponseCallback(res, err);
          }else{
            //async return the results in the same order they are executed
            if(datas && datas[2]){
              endResponseCallback(res, undefined,datas[2]);
            }else{
              endResponseCallback(res);
            }
          }
      });
...
function endResponseCallback(res, err, ok) {
