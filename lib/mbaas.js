...
var app = express.Router();
    ...
    app.post('/sync/:datasetId', function(req, res) {
      console.time('syncRequest')
      var params = {};
      var dataset_id = req.params.datasetId;
      console.time('syncRequestProcessParam')
      var params = paramsUtils.normalise(params, req);
      console.timeEnd('syncRequestProcessParam')

      console.time('doSync');
      fh.sync.invoke(dataset_id, params, function(err, ok) {
        console.timeEnd('doSync');
        endResponseCallback(req, res, err, ok);
        console.timeEnd('syncRequest')
        return;
      });
    });
