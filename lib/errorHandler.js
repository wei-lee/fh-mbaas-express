...
function handleUncaughtException(err){
  ...
  err = err || {};
  console.error(new Date().toISOString() + ' uncaughtException: ' + util.inspect(err));
...
function errHandler(err, req, res, next){
  console.log(new Date().toISOString() + " Internal error: " + util.inspect(err));
  console.error(new Date().toISOString() + " Internal error: " + util.inspect(err));
  ...
  // OPENSHIFT
    ...
    return function (message, cb){
      request({
        ...
        console.error(new Date().toISOString() + " Failed to publish error report: " + util.inspect(error));
        console.error(new Date().toISOString() + " Response body : " + body);
        return cb();
      });
    }
  }
  // MBAAS
    ...
    return function (message, cb){
      createMBaasEvent({
        ...
        if (err){
          console.error(new Date().toISOString() + ' Error creating event in the MBaaS: ' + util.inspect(err));
          return cb(err);
        } else {
          console.error(new Date().toISOString() + ' Create event sent to MBaaS: OK');
          return cb();
        }
      });
    }
