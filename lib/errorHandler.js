...
function handleUncaughtException(err){
  err = err || {};
  console.error('uncaughtException: ' + util.inspect(err));
...
function errHandler(err, req, res){
  ...
  var errorMessage = {
    msg: 'Internal error: ' + JSON.stringify(err),
  ...
  // OPENSHIFT
    ...
    return function (message, cb){
      request({
        ...
        console.error("Failed to publish error report: " + util.inspect(error));
  ...
  // MBAAS
    ...
    return function (message, cb){
      createMBaasEvent({
        ...
        if (err){
          console.error('Error creating event in the MBaaS: ' + util.inspect(err));
          return cb(err);
        } else {
          console.error('Create event sent to MBaaS: OK');
          return cb();
        }
      });
    }
