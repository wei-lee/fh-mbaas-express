  ...
  return {
    "forms": {
      ...
      "getSubmission": function(responseContent){
        var submissionArray = responseContent.submissions;

        if(submissionArray.length > 0){
          this.respondJSON(submissionArray[0]);
        } else {
          this.respondError(404, "No submission found");
        }
      },
      "getSubmissionFile": function(responseContent){
        ...
        if(responseContent.stream){
          ...
          this.responseError(404, "File not found");
        }
      },
