...
module.exports = function(req, res, params) {
  ...
  function verifyServiceAuthorisation() {
    // Relevant env vars
    var serviceAppEnv = process.env['FH_SERVICE_APP'];
    var serviceAppPublicEnv = process.env['FH_SERVICE_APP_PUBLIC'];
    var serviceAuthorisedProjectsEnv = process.env['FH_SERVICE_AUTHORISED_PROJECTS'];
    var projectId = process.env.FH_WIDGET;

    if (serviceAppEnv && serviceAppPublicEnv && serviceAuthorisedProjectsEnv) {
      var serviceApp = (serviceAppEnv === 'true');
      var serviceAppPublic = (serviceAppPublicEnv === 'true');
      var serviceAuthorisedProjects = serviceAuthorisedProjectsEnv.split(',');
      var accessingAppId = null;
      if (req.headers.hasOwnProperty(SERVICE_ID_KEY)) {
        accessingAppId = req.headers[SERVICE_ID_KEY];
      }
      
      // Add ourself as an authorisedProject, since should be able to call ourself
      // Dynamic docs in the Studio will also use this ID.
      serviceAuthorisedProjects.push(projectId);
      ...
      if (serviceAuthorisedProjects.indexOf(accessingAppId) != -1) {
        return true;
      }
    } else {
      // Not a service app, return
      return true;
    }
